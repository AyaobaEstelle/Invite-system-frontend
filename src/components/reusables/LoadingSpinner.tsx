...
const Spinner: React.FC<SpinnerProps> = ({
  size = "lg",
  color = "text-brand_secondary-600",
  fullScreen = true,
}) => {
  const sizeClasses = {
    sm: "w-5 h-5",
  ...
  const spinnerElement = (
    <div role="status" className="flex items-center justify-center">
      <svg
        aria-hidden="true"
        className={`${sizeClasses[size]} animate-spin ${color} fill-current`}
        viewBox="0 0 100 101"
        fill="none"
  ...
  if (fullScreen) {
    return (
      <div className="fixed inset-0 flex items-center justify-center bg-white/60 dark:bg-gray-900/70 backdrop-blur-sm z-50">
        {spinnerElement}
      </div>
    );
  }
